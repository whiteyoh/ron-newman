...
function renderScorePanel(agenticness){ if(!agenticness){scorePanel.innerHTML=''; return;} const fields=[['Agenticness score',`${agenticness.score||'n/a'}/10`],['Yegge alignment',`${agenticness.yegge_alignment_score||'n/a'}/10`],['Closest stage',agenticness.closest_yegge_stage||'n/a'],['Why this earns score',agenticness.explanation||'n/a'],['Why not higher',agenticness.not_higher_reason||'More verification and governance needed.'],['Try next',agenticness.try_next||'Use the lowest useful level, then add one extra check.']]; scorePanel.innerHTML=''; fields.forEach(([k,v])=>{ const c=createEl('article','score-card'); c.innerHTML=`<strong>${k}</strong><div>${v}</div>`; scorePanel.appendChild(c); }); }
...
async function runLevel(level){ if(!confirmedUseCase || runInProgress) return; runInProgress=true; setStatus('Running…'); clearRunPanels(); log.innerHTML=''; appendMessage('system','Running simulation. No answer needed.');
 ...
 renderScorePanel(data.agenticness); renderTheatre(data); renderTaskboard(data);
