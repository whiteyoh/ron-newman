...
USE_CASE_OPTIONS = {
    "uk_year10_teacher": "Use case for all levels: help a UK Year 10 teacher create engaging lesson plans for real classroom teaching. The lesson-plan output must be practical and classroom-ready for teachers and students, with clear teaching sequence, timings, activities, assessment, and differentiation. Revision-guide output must focus on actual revision guidance (what to revise, how to revise, and when to revise), and should be directly usable by a student or teacher to achieve the topic goals. The chat should ask clarifying questions when needed before drafting, including subject, specific topic, lesson length, student level, and constraints. Before generating a proper lesson plan, ask for the specific lesson/topic and intended lesson length; if key details are missing, ask follow-up questions to collect class constraints and context. Then produce a proper lesson plan based on the confirmed topic and length.",
