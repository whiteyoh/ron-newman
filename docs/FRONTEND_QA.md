# Frontend Manual QA Checklist

- [ ] Desktop hero layout is polished, clear, and readable.
- [ ] Mobile layout stacks cleanly with no horizontal scrolling.
- [ ] Keyboard navigation reaches all controls with visible focus styles.
- [ ] Skip link jumps to main content.
- [ ] Use case selection highlights active card.
- [ ] Confirming use case reveals level launcher.
- [ ] Running a level updates status strip and transcript.
- [ ] Agentic Theatre renders step cards with actor + status.
- [ ] Replay highlights steps and finishes with "Replay finished".
- [ ] Level 8 taskboard renders worker cards in status columns.
- [ ] Quiz result updates immediately when answers change.
- [ ] Download artifact button creates a text file after a run.
- [ ] Reduced motion preference disables/limits animation behavior.
- [ ] Missing API key fallback shows clear non-crashing status.
- [ ] Before selecting a level, helper text explains this is a simulated, read-only trace.
- [ ] During a run, the dashboard shows simulation-only messaging (no answer needed).
- [ ] Transcript and Agentic Theatre visibly include "Read-only trace" labels.
- [ ] Transcript labels use demo/trace wording (not Prompt/Question/Instruction as user asks).
- [ ] Only scenario/context/level controls appear interactive.
- [ ] Run each level and confirm no transcript line looks like the UI is waiting for user input.


## Yegge alignment update
This demo now presents **capability vs Yegge stage** with high-fidelity workshop simulations, explicit human approval gates, and clear non-production limits.
